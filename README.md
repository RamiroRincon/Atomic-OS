# ⚛️ Custom Bazzite Image

## 📥 Download

Get the latest stable build of this image. Our images are built automatically via GitHub Actions and hosted on Cloudflare R2 for maximum download speed.

| Platform | Format | Download Link |
| :--- | :--- | :--- |
| **Bare Metal / VM** | `.iso` | [🚀 Download ISO](https://pub-83d7652ee30a484ab7570aa3d522f52a.r2.dev/stable/Atomic-OS.iso) |
| **Virtualization** | `.qcow2` | [📦 Download QCOW2](https://pub-83d7652ee30a484ab7570aa3d522f52a.r2.dev/stable/Atomic-OS.qcow2) |

> [!TIP]
> **Which one do I need?**
> - **ISO:** Use this if you are installing on a physical laptop/desktop or a standard Virtual Machine (VirtualBox, VMware, GNOME Boxes).
> - **QCOW2:** Use this for high-performance virtualization in KVM, QEMU, or Proxmox.

---

### 🛠️ Build Status
![Build Status](https://img.shields.io/github/actions/workflow/status/RamiroRincon/Atomic-OS/build-disk.yml?branch=main&label=Latest%20Build)
![Storage](https://img.shields.io/badge/Hosted%20on-Cloudflare%20R2-orange)

*These links always point to the most recent successful stable build.*
